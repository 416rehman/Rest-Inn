...
export default function MinMaxFilterChoice({title, fields, filters, setFiltersHandler, minValue, maxValue, icon}: IProps) {
    ...
    const handleVariant = (names: string[]) => {
        return isActive(names, filters) ? 'filled' : 'outlined'
    }
    ...
    return (
        <PopoverStyled title={title} variant={handleVariant(fieldNames)} icon={icon}
                       onDelete={isActive(fieldNames, filters) ? () => {clearFilter(fieldNames)} : undefined}

        >
            <form onSubmit={(e) => {
                e.preventDefault();
                setFiltersHandler(pendingFilters);
            }}>
                <Stack gap={'0.5rem'} padding={'1rem 0.5rem 0.5rem 0.5rem'}>
                    <Stack direction={'row'} gap={'1rem'} flexWrap={'wrap'}>
                        {fields.map(field => (
                            <TextField key={field.name} label={field.label} type={'number'} name={field.name}
                                       size={'small'}
                                       value={pendingFilters[field.name]} onChange={onChangeHandler}
                                       onInput={onInputHandler}/>
                        ))}
                    </Stack>
                    <Stack direction={'row'} gap={'0.5rem'}>
                        {isActive(fieldNames, pendingFilters) &&
                            <Button type={'submit'} variant={'outlined'} color={'primary'}>Apply</Button>}
                        {isActive(fieldNames, filters) &&
                            <Button color={'error'} onClick={() => clearFilter(fieldNames)}>Clear</Button>}
                    </Stack>
                </Stack>
            </form>
        </PopoverStyled>)
